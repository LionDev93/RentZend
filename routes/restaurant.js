...
router.post(
  ...
  async( req, res ) => {
    try{
      ...
      console.log("req: dishInfo: ", dishInfo);
  ...
  async (req, res) => {
    try {
      ...
      upload(req, res, async function(err) {
        ...
        var file = req.file;
        const path = file.destination.replace("./static", "") + file.filename;

        const dish_id = req.body.dish_id;
        const dish = await Dish.findByIdAndUpdate(
          mongoose.Types.ObjectId(dish_id),
          {
            image: path,
          }
        )
        return res.status(200).json({image: dish.image});
      });
    } catch (err) {
      console.log("create dish err:", err);
    return res.status(500).json();
